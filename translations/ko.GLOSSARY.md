...
#### 군중 사건 표시 결함 수정 (2026-07-29, 사용자 발견)
- 발견: 사건 위키 페이지가 효과의 type/prerequisite 를 렌더링하지 않아 「광기의 밤」이
  조건(좀비 6구 이상) 없이 "주인공 사망"으로 보였고, mob=0 군중 사건은 `{#if incident.mob}` falsy 버그로
  군중 표기 자체가 숨겨짐. 또한 배열형 translationKey 가 스키마 추출에서 누락되어
  `Mob incident ({mob})` 등 15개 키가 번역 불가능 상태였음 (모두 upstream 코드 결함).
- 조치: 포크에서 코드 3건 수정 + 신규 키 15개 번역 + 군중 사건 효과 문구 3건을 공식 참조표 문구로 정정
  (좀비 6명→6구, 라운드 종료 단계 문형, 저주 카드 명시). Loss condition 계열 type 라벨의 중복 대괄호 정리.
- 날짜 / 결정자: 2026-07-29 / 사용자 지적, Claude 수정
